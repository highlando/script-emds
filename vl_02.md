 ...
 * falls das lineare Gleichungssystem $\Phi (\mathbf x)w = \mathbf y$ eine L&ouml;sung hat $w$, ist das auch eine L&ouml;sung unserer Minimierung
 * im typischen Falle aber ist allerdings $N\gg n$ und das System &uuml;berbestimmt ($n=N$ w&uuml;rde ein overfitting bedeuten...) sodass wir keine L&ouml;sung erwarten k&ouml;nnen.
...
\begin{equation*}
\begin{split}
w \mapsto \frac 12 \|\mathbf y - \Phi(\mathbf x) \|^2 &= \frac 12 (\mathbf y - \Phi(\mathbf x)w)^T(\mathbf y - \Phi(\mathbf x)w)  \\ 
&= \frac 12 [\mathbf y^T\mathbf y -y^T \Phi(\mathbf x)w - w^T \Phi(\mathbf x)^T\mathbf y  + w^T \Phi(\mathbf x)^T\Phi(\mathbf x)w] \\
...
W&auml;ren $w$ und $y$ keine Vektoren sondern einfach reelle Zahlen, w&auml;re das hier eine Parabelgleichung $aw^2 + bw + c$ mit $a>0$, die immer eine Minimalstelle hat.
 ...
 * $b_i$'s werden als *linear unabh&auml;ngige* (im Sinne von Funktionenr&auml;umen) gewählt (was die lineare unabh&auml;ngigket der $b_i(\mathbf x)$ impliziert.
