...
Zeigen Sie, dass die quadrierten Singulärwerte einer Matrix \(A\in \mathbb R^{m\times n}\), \(m>n\), genau die Eigenwerte der Matrix \(A^TA\) sind und in welcher Beziehung sie mit den Eigenwerten von \(AA^T\) stehen. \textbf{Hinweis}: hier ist ``\(m>n\)'' wichtig.
...
durch. Berechnen Sie die mittlere quadratische Abweichung \(\frac{1}{N}\sum_{i=1}^N\|y_i - f(x_i)\|^2\) für alle Fälle und plotten Sie die Daten zusammen mit der aus der Regression erhaltenen Funktion. Beispielsweise so:
...
\hypertarget{truncated-svd-p}{%
\subsection{Truncated SVD (P)}\label{truncated-svd-p}}

Berechnen und plotten sie die Singulärwerte einer \(1000\times 100\) Matrix mit zufälligen Einträgen und die einer Matrix mit ``echten'' Daten (hier Simulationsdaten einer Stroemungssimulation)\footnote{wird noch online bereitgestellt}. Berechnen sie den Fehler der \emph{truncated SVD} \(\|A-A_{\hat r}\|\) für \(\hat r = 10, 20, 40\) für beide Matrizen (und vergleichen sie mit der gegebenen Formel).
...
\NormalTok{randmat }\OperatorTok{=}\NormalTok{ np.random.randn(}\DecValTok{1000}\NormalTok{, }\DecValTok{100}\NormalTok{)}
