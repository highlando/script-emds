...
durch. Berechnen Sie die mittlere quadratische Abweichung $\frac{1}{N}\sum_{i=1}^N\|y_i - f(x_i)\|^2$ f&uuml;r alle F&auml;lle und plotten Sie die Daten zusammen mit der aus der Regression erhaltenen Funktion. Beispielsweise so:
...
Berechnen und plotten sie die Singul&auml;rwerte einer $1000\times 100$ Matrix mit zuf&auml;lligen Eintr&auml;gen und die einer Matrix mit "echten" Daten (hier Simulationsdaten einer Stroemungssimulation)^[wird noch online bereitgestellt].  Berechnen sie den Fehler der *truncated SVD* $\|A-A_{\hat r}\|$ f&uuml;r $\hat r = 10, 20, 40$ f&uuml;r beide Matrizen (und vergleichen sie mit der gegebenen Formel).
...
randmat = np.random.randn(1000, 100)
