...
Die Grundlage ist der \href{https://en.wikipedia.org/wiki/Iris_flower_data_set}{\emph{Iris Datensatz}}, der ein gern genommene Grundlage für die Illustration der Datenanalyse ist. Die Daten wurden von einem Statistiker und Biologen erhoben und beinhalten 4 verschiedene Merkmale (engl. \emph{features}) von einer Stichprobe von insgesamt 150 Schwertlilien die 3 verschiedenen Klassen zugeordnet werden können oder sollen (Fachbegriff hier: \emph{targets}). Im Beispiel werden die Klassen mit \texttt{0,\ 1,\ 2} codiert und beschreiben die Unterarten \texttt{setosa}, \texttt{versicolor} und \texttt{virginica} der Schwertlilien. Die Merkmale sind gemessene Länge und Breite des Kelchblattes (hier \emph{sepal}) und des Kronblattes (hier \emph{petal}).
...
\begin{figure}
\hypertarget{fig:05-iris-allpairs}{%
\centering
\includegraphics[width=0.75\textwidth,height=\textheight]{bilder/05-all-pairs.png}
\caption{Iris Datenset 2D plots}\label{fig:05-iris-allpairs}
}
\end{figure}
...
Wir bemerken, dass auf der Hauptdiagonalen die Varianzen in Koordinatenrichtung stehen und in den Zeilen oder Spalten ein Mass daf"ur, wie bspw. \({\mathbf{x} _ i}\) und \({\mathbf{x} _ j}\) korreliert sind. Große Zahlen bedeuten eine große Varianz oder eine starke Korrelation und umgekehrt. F"ur die Datenanalyse k"onnen wir \(\operatorname{Cov}({\mathbf{X}})\) wie folgt heranziehen.
...
\tightlist
\item
  \(\operatorname{Cov}(X)\) eine Diagonalmatrix ist --- damit wären alle Richtungen in den Daten \emph{unkorreliert} und könnten \emph{unabhängig} voneinander betrachtet werden\footnote{wir dürfen aber nicht vergessen, dass Daten tüpischerweise nur eine Stichprobe von Beobachtungen eines Phänomens sind. Die Unabhängigkeit in den \emph{features} gilt also nur für die gesammelten Daten aber in der Regel nicht für das Phänomen. Für normalverteilte Prozesse liefern die daten-basiert ermittelten Hauptrichtungen jedoch auch die Hauptrichtungen des zugrundeliegenden Phänomens} -- und in der
\item
  die neuen Varianzen nach Größe geordnet sind gleichzeitig die verfügbare Varianz maximal in wenigen Richtungen konzentrieren.
\end{itemize}
...
In der Praxis sind schon allein durch Messfehler exakte lineare Abhängigkeiten sowiedurch die näherungsweise Bestimmung auf dem Computer das Auftreten von \(\sigma_k =0\) quasi ausgeschlossen. Stattdessen werden Schwellwerte definiert, unterhalb derer die SVD abgeschnitten wird.
...
\hypertarget{am-beispiel-der-iris-daten}{%
\section{Am Beispiel der Iris Daten}\label{am-beispiel-der-iris-daten}}

Wir stellen die Daten noch einmal zentriert dar:

\begin{figure}
\hypertarget{fig:05-iris-allpairs-cntrd}{%
\centering
\includegraphics[width=0.75\textwidth,height=\textheight]{bilder/05-all-pairs-cntrd.png}
\caption{Iris Daten Zentriert}\label{fig:05-iris-allpairs-cntrd}
}
\end{figure}

Für die Kovarianzmatrix der Iris Daten erhalten wir:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Covariance matrix: }
\NormalTok{[[ }\FloatTok{0.6857} \FloatTok{-0.0424}  \FloatTok{1.2743}  \FloatTok{0.5163}\NormalTok{]}
\NormalTok{ [}\OperatorTok{-}\FloatTok{0.0424}  \FloatTok{0.19}   \FloatTok{-0.3297} \FloatTok{-0.1216}\NormalTok{]}
\NormalTok{ [ }\FloatTok{1.2743} \FloatTok{-0.3297}  \FloatTok{3.1163}  \FloatTok{1.2956}\NormalTok{]}
\NormalTok{ [ }\FloatTok{0.5163} \FloatTok{-0.1216}  \FloatTok{1.2956}  \FloatTok{0.581}\NormalTok{ ]]}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{Out: [}\FloatTok{25.1}     \FloatTok{6.0131}  \FloatTok{3.4137}  \FloatTok{1.8845}\NormalTok{]}
\end{Highlighting}
\end{Shaded}

Zwar ist hier kein Singulärwert nah an der Null, allerdings beträgt der Unterschied zwischen dem größten und dem kleinsten schon eine 10er Potenz, was auf eine starke Dominanz der ersten Hauptachsen hinweist.

In der Tat, plotten wir die Daten in den Koordinaten der Hauptachsen (also \(\tilde {\mathbf{X}}\)), zeigen Bilder der \(v_1\) oder \(v_2\)-Koordinaten klare Tendenzen in den Daten, während die Plots der übrigen Richtungen wie eine zufällige Punktwolke aussieht, vgl. Abbildung \ref{fig:05-iris-allpairs-pcs}.

\begin{figure}
\hypertarget{fig:05-iris-allpairs-pcs}{%
\centering
\includegraphics[width=0.75\textwidth,height=\textheight]{bilder/05-all-pairs-pcs.png}
\caption{Iris Datenset 2D plots in Hauptachsenkoordinaten}\label{fig:05-iris-allpairs-pcs}
}
\end{figure}
...
\begin{figure}
\hypertarget{fig:05-iris-allpairs-k2-rec}{%
\centering
\includegraphics[width=0.75\textwidth,height=\textheight]{bilder/05-all-pairs-k2-rec.png}
\caption{Iris Datenset -- rekonstruiert von \(\hat k =2\) Hauptachsenkoordinaten}\label{fig:05-iris-allpairs-k2-rec}
}
\end{figure}
...
\hypertarget{covarianzen}{%
\subsection{Covarianzen}\label{covarianzen}}

Erzeugen Sie für \texttt{N} aus \texttt{Nl\ =\ {[}10,\ 100,\ 1000{]}} zufällige Vektoren \texttt{x} und \texttt{y} der Länge \texttt{N} und berechnen Sie jeweils die Covarianzmatrix für den Datensatz \texttt{X\ =\ {[}x,\ x,\ x*y,\ y{]}}. Interpretieren sie die Ergebnisse.

\hypertarget{iris-datensatz-targets-plotten}{%
\subsection{Iris Datensatz -- Targets Plotten}\label{iris-datensatz-targets-plotten}}

Laden Sie die \href{bilder/iris-data.json}{Iris Datensatz} (hier als \texttt{json} file bereitgestellt) und plotten sie \texttt{petal\ width} versus \texttt{sepal\ length} für die drei Spezies \texttt{setosa}, \texttt{versicolor}, \texttt{virginica} in drei separaten Grafiken.
...
\ControlFlowTok{with} \BuiltInTok{open}\NormalTok{(}\StringTok{'iris-data.json'}\NormalTok{, }\StringTok{'r'}\NormalTok{) }\ImportTok{as}\NormalTok{ f:}
...
\hypertarget{iris-datensatz-2d-plots}{%
\subsection{Iris Datensatz -- 2D plots}\label{iris-datensatz-2d-plots}}

Erzeugen sie Abbildung \ref{fig:05-iris-allpairs} zum Beispiel über ein \texttt{4x4} Feld von subplots
...
\hypertarget{hauptachsentransformation-1}{%
\subsection{Hauptachsentransformation}\label{hauptachsentransformation-1}}
...
Zeigen sie, dass f"ur zentrierte Datens"atze \({\mathbf{X}}\in \mathbb R^{N\times n}\) gilt, dass
\begin{equation*}
