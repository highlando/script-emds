...
{\centering \includegraphics[width=0.8\linewidth]{bilder/06-Figure_10101} \includegraphics[width=0.8\linewidth]{bilder/06-Figure_101} \includegraphics[width=0.8\linewidth]{bilder/06-Figure_111} \includegraphics[width=0.8\linewidth]{bilder/06-Figure_121} 
...
{\centering \includegraphics[width=0.8\linewidth]{bilder/06-L-curve} 
...
\NormalTok{        cntrdlist }\OperatorTok{=}\NormalTok{ get_centers(datamatrix, K}\OperatorTok{=}\NormalTok{K, labels}\OperatorTok{=}\NormalTok{clabels)}
\NormalTok{        clabels, newerror }\OperatorTok{=}\NormalTok{ get_labels(datamatrix, centroids}\OperatorTok{=}\NormalTok{cntrdlist)}
        \ControlFlowTok{if}\NormalTok{ np.}\BuiltInTok{abs}\NormalTok{(olderror}\OperatorTok{-}\NormalTok{newerror) }\OperatorTok{<}\NormalTok{ itertol:}
            \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f'iteration }\SpecialCharTok{\{}\NormalTok{kkk}\SpecialCharTok{\}}\SpecialStringTok{: change in error below }\SpecialCharTok{\{}\NormalTok{itertol}\SpecialCharTok{:.2e\}}\SpecialStringTok{'}\NormalTok{)}
            \ControlFlowTok{break}
        \ControlFlowTok{else}\NormalTok{:}
\NormalTok{            olderror }\OperatorTok{=}\NormalTok{ newerror}
    \ControlFlowTok{return}\NormalTok{ cntrdlist, clabels, newerror}


\KeywordTok{def}\NormalTok{ plot_cluster_data(datamatrix, labels, cntrdlist,}
\NormalTok{                      xlabel}\OperatorTok{=}\StringTok{'feature'}\NormalTok{, ylabel}\OperatorTok{=}\StringTok{'other feature'}\NormalTok{,}
\NormalTok{                      fignum}\OperatorTok{=}\DecValTok{10101}\NormalTok{, figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{4}\NormalTok{), titlestr}\OperatorTok{=}\StringTok{'Clustering'}\NormalTok{):}
\NormalTok{    plt.figure(fignum, figsize}\OperatorTok{=}\NormalTok{figsize)}

    \CommentTok{# Set the colors for the plots}
\NormalTok{    clm }\OperatorTok{=}\NormalTok{ np.linspace(}\FloatTok{0.2}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\BuiltInTok{len}\NormalTok{(cntrdlist))}
\NormalTok{    plt.rcParams[}\StringTok{"axes.prop_cycle"}\NormalTok{] }\OperatorTok{=} \OperatorTok{\textbackslash{}}
\NormalTok{        plt.cycler(}\StringTok{"color"}\NormalTok{, plt.cm.plasma(clm))}
    \CommentTok{# done setting colors}

    \ControlFlowTok{for}\NormalTok{ kkk }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(}\BuiltInTok{len}\NormalTok{(cntrdlist)):}
\NormalTok{        cdata }\OperatorTok{=}\NormalTok{ datamatrix[labels }\OperatorTok{==}\NormalTok{ kkk, :]}
\NormalTok{        plt.plot(cdata[:, }\DecValTok{0}\NormalTok{], cdata[:, }\DecValTok{1}\NormalTok{], }\StringTok{'.'}\NormalTok{, alpha}\OperatorTok{=}\NormalTok{.}\DecValTok{7}\NormalTok{)}
    \ControlFlowTok{for}\NormalTok{ kkk }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(}\BuiltInTok{len}\NormalTok{(cntrdlist)):}
\NormalTok{        plt.plot(cntrdlist[kkk][}\DecValTok{0}\NormalTok{], cntrdlist[kkk][}\DecValTok{1}\NormalTok{],}
                 \StringTok{'s'}\NormalTok{, label}\OperatorTok{=}\SpecialStringTok{f'$c_}\SpecialCharTok{\{}\NormalTok{kkk}\OperatorTok{+}\DecValTok{1}\SpecialCharTok{\}}\SpecialStringTok{$'}\NormalTok{)}
