...
\item
  falls das lineare Gleichungssystem \(\Phi (\mathbf x)w = \mathbf y\) eine Lösung hat \(w\), ist das auch eine Lösung unserer Minimierung
\item
  im typischen Falle aber ist allerdings \(N\gg n\) und das System überbestimmt (\(n=N\) würde ein overfitting bedeuten\ldots{}) sodass wir keine Lösung erwarten können.
\item
...
Wären \(w\) und \(y\) keine Vektoren sondern einfach reelle Zahlen, wäre das hier eine Parabelgleichung \(aw^2 + bw + c\) mit \(a>0\), die immer eine Minimalstelle hat.
...
\item
  \(b_i\)'s werden als \emph{linear unabhängige} (im Sinne von Funktionenräumen) gewählt (was die lineare unabhängigket der \(b_i(\mathbf x)\) impliziert.
\end{itemize}
